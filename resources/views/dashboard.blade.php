            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-2xl p-6 mb-12 border border-gray-700">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 divide-x-0 md:divide-x divide-gray-700">
                    ...
                    <div class="text-center px-4 border-l border-gray-700">
                        <span class="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Asignados</span>
                        ...
                        <div class="relative h-48 bg-gray-900 overflow-hidden">
                            ...
                            <div class="absolute top-4 left-4">
                                <span class="px-3 py-1 rounded-lg text-[10px] font-black tracking-widest border
                                    {{ $vehicle->status === 'available' ? 'text-green-400 bg-green-900/40 border-green-500/50' : '' }}
                                    {{ $vehicle->status === 'workshop' ? 'text-red-400 bg-red-900/40 border-red-500/50' : '' }}
                                    {{ $vehicle->status === 'maintenance' ? 'text-yellow-400 bg-yellow-900/40 border-yellow-500/50' : '' }}
                                    {{ $vehicle->status === 'assigned' ? 'text-blue-400 bg-blue-900/40 border-blue-500/50' : '' }}">
                                    @switch($vehicle->status)
                                        ...
                                        @default {{ strtoupper($vehicle->status) }}
                                    @endswitch
                                </span>
                            </div>
                        </div>
                ...
                <div class="p-8">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div class="space-y-4 text-white">
                            ...
                            <div><span class="block text-[10px] text-gray-400 uppercase tracking-widest font-bold">Año / Kilometraje</span><span class="text-lg font-mono" x-text="viewingVehicle.year + ' • ' + viewingVehicle.mileage + ' KM'"></span></div>
                        </div>
                    </div>
