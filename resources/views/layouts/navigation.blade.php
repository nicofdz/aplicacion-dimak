                                ...
                                @else
                                    @foreach(Auth::user()->notifications as $notification)
                                        <a href="{{ route('notifications.read', $notification->id) }}"
                                            class="group block px-4 py-4 text-sm bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700 last:border-0 transition duration-150 {{ $notification->read_at ? 'opacity-75' : '' }}">
                                            <div class="flex items-start space-x-3">
                                                <div class="flex-shrink-0 pt-1">
                                                    @if($notification->data['type'] == 'danger')
                                                        <span
                                                            class="flex items-center justify-center h-8 w-8 rounded-full bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400">
                                                    ...
                                                    @else
                                                        <span
                                                            class="flex items-center justify-center h-8 w-8 rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 dark:text-yellow-400">
                                                            <svg class="w-5 h-5" fill="none" stroke="currentColor"
                                                                viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round"
                                                                    stroke-width="2"
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    <p
                                                        class="text-sm font-semibold text-gray-900 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                                                        {{ $notification->data['message'] }}
                                                    </p>
                                                    <p
                                                        class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-0.5 uppercase tracking-wide">
                                                        {{ $notification->data['brand_model'] }} <span class="mx-1">•</span>
                                                        {{ $notification->data['plate'] }}
                                                    </p>
                                                    <p
                                                        class="mt-1 text-[10px] text-gray-400 dark:text-gray-500 flex items-center">
