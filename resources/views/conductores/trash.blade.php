    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">
                {{ __('Conductores en Papelera') }}
            </h2>
            <a href="{{ route('conductores.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-500 transition ease-in-out duration-150">
                ...
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-red-50 dark:bg-red-900/20">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-400 uppercase tracking-widest">Eliminado</th>
                            <th class="px-6 py-3 text-right text-xs font-bold text-gray-400 uppercase tracking-widest">Acciones</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        @forelse($conductoresEliminados as $conductor)
                            <tr class="opacity-75 hover:opacity-100 transition-opacity">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="h-10 w-10 flex-shrink-0 bg-gray-600 rounded-full flex items-center justify-center overflow-hidden grayscale">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-400">
                                    {{ $conductor->deleted_at->diffForHumans() }}
                                </td>

                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end items-center space-x-4">
                                        <form action="{{ route('conductores.restore', $conductor->id) }}" method="POST" class="inline">
                                            @csrf
                                            <button type="submit" class="text-green-400 hover:text-green-300 transition duration-150">
                                        ...
                                        <button @click="deleteAction = '{{ route('conductores.force-delete', $conductor->id) }}'; openDeleteModal = true" 
                ...
                <div class="p-8">
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-300 mb-8">
                            {{ __('Esta acción NO se puede deshacer. Se eliminará el conductor y su foto del sistema.') }}
                        </p>
                    </div>
