    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-2">
                ...
                <a href="{{ route('conductores.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 transition">
                    + Nuevo Conductor
                </a>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700/30 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end items-center gap-4">
                                        <button @click="viewingConductor = { nombre: '{{ $conductor->nombre }}', cargo: '{{ $conductor->cargo }}', depto: '{{ $conductor->departamento }}', vencimiento: '{{ $conductor->fecha_licencia->format('d/m/Y') }}', foto: '{{ $conductor->fotografia ? asset('storage/' . $conductor->fotografia) : '' }}' }; openViewModal = true" class="text-green-400 hover:text-green-300">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                        </button>
                                        <a href="{{ route('conductores.edit', $conductor) }}" class="text-blue-400 hover:text-blue-300">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                        </a>
                                        <button @click="deleteAction = '{{ route('conductores.destroy', $conductor) }}'; openDeleteModal = true" class="text-red-400 hover:text-red-300">
