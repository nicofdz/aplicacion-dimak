    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-100 leading-tight">
                {{ __('Salas en Papelera') }}
            </h2>
            <a href="{{ route('rooms.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-500 transition ease-in-out duration-150">
                            ...
                            <tr class="opacity-75 hover:opacity-100 transition-opacity">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="h-10 w-10 flex-shrink-0 bg-gray-600 rounded-full flex items-center justify-center overflow-hidden grayscale">
                                        @if($room->image_path)
                                            <img src="{{ Storage::url($room->image_path) }}" class="h-full w-full object-cover">
                                        @else
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex justify-end items-center space-x-4">
                                        <form action="{{ route('rooms.restore', $room->id) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="text-green-400 hover:text-green-300 transition duration-150" title="Restaurar">
                                        ...
                                        <button @click="deleteAction = '{{ route('rooms.force-delete', $room->id) }}'; openDeleteModal = true" 
                                                class="text-red-500 hover:text-red-400 transition duration-150" title="Eliminar para siempre">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="p-8">
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-300 mb-8">
                            {{ __('Esta acción NO se puede deshacer. Se eliminará la sala y su foto del sistema.') }}
                        </p>
                    </div>
